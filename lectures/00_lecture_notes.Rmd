...
ex: Let $F(\cdot)$ be the CDF $\mathcal{N}(\mu, \sigma^2)$ but $\mu, \sigma^2$ are unknown.

**How to estimate $se(\hat{\theta})$**

Bootstrap sample 1: $X_1^*, \dots, X_n^* \sim F(\cdot; \hat \theta) \longrightarrow \hat{\theta}_1^*$ 
...
Recall $(X, Y)' \sim \text{bivar}\mathcal{N} \left(\mu_x, \mu_y, \sigma^2_x, \sigma^2_y\right)$