...
fishery <- read.csv("../../../problems/midterm/datasets/fishery.csv")*1000
n <- nrow(fishery)
fishery
```
...
## Bootstrap estimate of SE
  ...
  * According to this reference. A sufficient statistic for the parameter of the Poisson distribution, $\lambda$ is $\sum_{i = 1}^n x_i$. Since it is sufficient to know the average if we know the sum (given the sample), then we can also take the mean as another sufficient statistic.
...
We are $95\%$ confident that the true value of $\theta$ is between `r ci[1]` and `r ci[2]`. If instead we use sum for the sufficient statistic, then the CI will be a lot wider, with the sample statistic, not even making it within the interval from `r quantile(suff2_theta_star,.025)` to `r quantile(suff2_theta_star,.975)`. My guess is that the sum as sufficient statistic is centered on the difference of the sums, not the difference of means. 
