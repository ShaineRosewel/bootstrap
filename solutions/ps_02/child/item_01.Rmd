...
```{r}
set.seed(seed)
res = sapply(1:B1, function(.){bootstrap_fn(estimate = "median", meth = "bca")})
ses <- unlist(res[1,])
tbs <- unlist(res[2,])
...
```{r}
set.seed(seed)
res_median = sapply(1:B1, function(.){bootstrap_fn(estimate = "median", meth = "percentile")})
...
```{r}
set.seed(seed)
res_mean = sapply(1:B1, function(.){bootstrap_fn(estimate = "mean", meth = "percentile")})
